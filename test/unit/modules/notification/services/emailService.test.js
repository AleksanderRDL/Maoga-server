  ...
  describe('sendNotificationEmail', () => {
    ...
    it('should return error if sendMail fails', async () => {
      ...
      const options = {
        to: 'recipient@example.com',
        subject: 'Test Subject',
        data: { type: 'default', message: 'Test' }
      };
      const result = await emailService.sendNotificationEmail(options);
      expect(result.success).to.be.false;
  ...
  describe('sendBulkEmails', () => {
      ...
      const recipients = [
        {
          ...
          data: { name: 'User1', type: 'default', message: 'Hi User1' }
        },
        {
          email: 'test2@example.com',
          data: { name: 'User2', type: 'default', message: 'Hi User2' }
        }
      ];
    ...
    it('should handle partial failures in bulk sending', async () => {
      ...
      mockTransporter.sendMail
        .onFirstCall()
        .resolves({ messageId: 'success-id' })
        .onSecondCall()
        .rejects(new Error('SMTP send failed'));
