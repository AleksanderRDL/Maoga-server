...
describe('NotificationService Unit Tests', () => {
    ...
    describe('createNotification', () => {
        ...
        beforeEach(() => {
            ...
            dispatchNotificationStub = sandbox.stub(notificationService, 'dispatchNotification').resolves();
        });
        ...
        it('should create a notification with friend_request type and determine correct channels', async () => {
            ...
            const notification = await notificationService.createNotification(mockUser._id.toString(), notificationData);
    ...
    describe('dispatchNotification', () => {
        ...
        it('should add job to push queue if in deliveryChannels', async () => {
            ...
            await notificationService.dispatchNotification(mockNotification, mockUser);
            expect(addJobStub.calledOnceWith('push', { notificationId: mockNotification._id.toString(), userId: mockUser._id.toString() })).to.be.true;
        });

        it('should add job to email queue if in deliveryChannels', async () => {
            mockNotification.deliveryChannels = ['email'];
            await notificationService.dispatchNotification(mockNotification, mockUser);
            expect(addJobStub.calledOnceWith('email', { notificationId: mockNotification._id.toString(), userId: mockUser._id.toString() })).to.be.true;
        });
    ...
    describe('markAsRead, markManyAsRead, markAllAsRead', () => {
        ...
        it('markManyAsRead should update multiple notifications and emit count', async () => {
            const notificationIds = [new mongoose.Types.ObjectId().toString(), new mongoose.Types.ObjectId().toString()];
        ...
        it('markAllAsRead should update all unread notifications and emit count 0', async () => {
            ...
            expect(Notification.updateMany.calledOnceWith({ userId, status: 'unread' }, sinon.match.object)).to.be.true;
