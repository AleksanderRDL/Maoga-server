...
describe('Cross-Service Notification Trigger Integration Tests', () => {
  ...
  beforeEach(async () => {
    ...
    [user1, user2, user3] = await Promise.all([
      ...
      authService
        .register({ email: 'user3@example.com', username: 'user3', password: 'Password123!' })
        .then((r) => r.user)
    ]);
  ...
  it('should create a notification when a match is found and finalized', async () => {
    ...
    const user1Call =
      notificationArgsUser1[0] === user1._id.toString()
        ? notificationArgsUser1
        : notificationArgsUser2;
    const user2Call =
      notificationArgsUser2[0] === user2._id.toString()
        ? notificationArgsUser2
        : notificationArgsUser1;
